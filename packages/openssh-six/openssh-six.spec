...
# Original source, name changed for CLIP build reasons
#Source0: ftp://ftp.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-%{version}.tar.gz
Source0: ftp://ftp.openbsd.org/pub/OpenBSD/OpenSSH/portable/openssh-six-%{version}.tar.gz
%if ! %{no_x11_askpass}
Source1: http://www.jmknoble.net/software/x11-ssh-askpass/x11-ssh-askpass-%{aversion}.tar.gz
Source2: rhel6_pam_sshd
%else
Source1: rhel6_pam_sshd
%endif
License: BSD
Group: Applications/Internet
BuildRoot: %{_tmppath}/%{name}-%{version}-buildroot
Obsoletes: ssh, openssh
%if %{build6x}
...
%if ! %{build6x}
BuildRequires: glibc-devel, pam, libselinux-devel
%else
...
Group: Applications/Internet
Obsoletes: ssh-clients, openssh-clients
Provides: openssh-clients

%package server
Summary: The OpenSSH server daemon.
Group: System Environment/Daemons
Obsoletes: ssh-server, openssh-server
Requires: %{pkgname} = %{version}-%{release}, chkconfig >= 0.9, tcp_wrappers, libselinux
%if ! %{build6x}
...
install -m644  $RPM_SOURCE_DIR/rhel6_pam_sshd  $RPM_BUILD_ROOT/etc/pam.d/sshd
%else
...
sed -i -e 's/#\s*UsePAM .*/UsePAM yes/' /etc/ssh/sshd_config

echo "
# Secure Secure Shell settings
...
%post clients
echo "# Secure Secure Shell settings
# https://stribika.github.io/2015/01/04/secure-secure-shell.html
    
Host *
# more compatiable than next line
